...
const imagesDir = path.join(process.cwd(), 'public/assets/procedural');
...
fs.readdir(imagesDir, (err, files) => {
  ...
  const tsxContent = `export const ImageNames: string[] = ${JSON.stringify(imageNames)};`;

  // Сохраняем в src/components/imageNames.tsx
  fs.writeFile(path.join(outputDir, 'ImageNames.tsx'), tsxContent, (err) => {
    if (err) {
